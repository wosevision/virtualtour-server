...
module.exports = function(router) {
  ...
  /**
   * POST '/hotspots/:id'
   * Receives a POST request with data of the hotspot to update, updates db, responds back
   * @param  {String} req.param('id'). The _id to update
   * @param  {Object} req. An object containing the data to update
   * @return {Object} JSON
   */
  router.post('/hotspots/:id', function(req, res){
  ...
  /**
   * DELETE '/hotspots/:id'
   * Receives a DELETE request specifying the hotspot to delete
  ...
  router.delete('/hotspots/:id', function(req, res){
    ...
    Hotspot.findByIdAndRemove(requestedId,function(err, data){

      // if err or no pano found, respond with error 
      if (err) {
        var error = {message: 'Error deleting hotspot', error: err};
         return res.status(400).json(error);
      }
      if (data == null) {
        var error = {message: 'hotspot not found'};
